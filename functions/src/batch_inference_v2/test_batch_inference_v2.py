...
def assert_batch_predict(
    n_features,
    batch_inference_run,
    with_monitoring=False,
    project_name="batch-infer-test",
):
    ...
    assert (
        len(batch_inference_run.artifact("prediction").as_df().columns)
        == n_features + 1
    )
    if with_monitoring:
        ...
        endpoint = get_or_create_model_endpoint(
            project=project.name, model_endpoint_name="my_cool_endpoint"
        )
...
def test_batch_predict():
    project = get_or_create_project("batch-infer-test", context="./", user_project=True)
    # Configure test:
    n_samples = 5000
    n_features = 20
    ...
    project.set_model_monitoring_credentials(
        endpoint_store_connection="v3io", tsdb_connection="v3io", stream_path="v3io"
    )
    ...
    # Check the logged results:
    assert_batch_predict(
        n_features=n_features,
        batch_inference_run=batch_inference_run,
...
class TestBatchInferUnitTests:
    ...
    def setup_method(self):
        ...
        self.context = mlrun.get_or_create_ctx(
            datetime_str, project=self.project.metadata.name, upload_artifacts=True
        )
    ...
    @pytest.mark.parametrize(
        "sample_type", [mlrun.DataItem, list, dict, pd.DataFrame, np.ndarray]
    )
    def test_infer_sample_types(self, sample_type):
        ...
        model = self.project.log_model(
            ...
            model_file=f"model-{uuid.uuid4()}.pkl",
            framework="xgboost",
            training_set=train_set_to_log,
            label_column="target_label",
        )

        dataset = self.project.log_dataset(f"dataset-{uuid.uuid4()}", df=x_test)
        z_test = train_set_to_log * 5
        model_endpoint_sample_set = self.project.log_dataset(
            f"model-endpoint-sample-set{uuid.uuid4()}", df=z_test
        )
        ...
        infer(
            ...
            dataset=dataset.to_dataitem().as_df(),
            model_path=model.uri,
            model_endpoint_sample_set=sample,
            feature_columns=list(
                model_endpoint_sample_set.to_dataitem().as_df().columns
            ),
            label_columns="target_label",
        ...
        batch_inference_run = self.project.list_runs(
            name=self.context.name
        ).to_objects()[0]
        mlrun.get_run_db().update_run(
            updates={"status.state": "completed"}, uid=batch_inference_run.uid()
        )
        assert_batch_predict(
            n_features=n_features,
            batch_inference_run=batch_inference_run,
